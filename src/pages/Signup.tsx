...
export default function Signup() {
  ...
  const {
    ...
    formState: { isValid: isNicknameValid },
  } = useForm({ resolver: zodResolver(nicknameSchema), mode: 'onChange' });
  ...
  return (
    <div className="flex flex-col min-h-screen items-center justify-center bg-black text-white relative">
      ...
      {step === 2 && (
        <form
          onSubmit={handlePasswordSubmit(() => setStep(3))}
          className="w-80 flex flex-col gap-4"
        >
      ...
      {step === 3 && (
        <form
          onSubmit={handleNicknameSubmit((data) => {
            alert(`회원가입 완료! 닉네임: ${data.nickname}`);
            navigate('/');
          })}
        ...
        >
          ...
          <button
            ...
            className={`p-2 w-full rounded ${isNicknameValid ? 'bg-pink-500' : 'bg-gray-700'}`}
          >
            회원가입 완료
          </button>
        </form>
